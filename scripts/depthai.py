...
while True:
    ...
    for packet in data_packets:
        ...
        if packetData is None:
            ...
            nn_frame,location_x,location_y,location_z,label_text = show_nn(nnet_prev["entries_prev"][camera], frame, labels=labels, config=config)
            label_name = label_text
            if str(location_z)!="[]":
                print("ok for location z")
                #rounded_value = round(location_z,2)
                locationz.append(str(location_z))
            if str(location_y)!="[]":
                print("ok for location y")
                #rounded_value = round(location_y,2)
                locationy.append(str(location_y))
    ...
    if label_name == "pest":
        if len(locationz)!=0 and len(locationy)!=0:
            #location = [float(n) for n in location.split()] # alternatively
            locationz = [float(x) for x in locationz]
            avgz = sum(locationz) / len(locationz)
            avg_roundz = round(avgz,1)
            print("average value of z ", avg_roundz*0.01)
            locationy = [float(x) for x in locationy]
            avgy = sum(locationy) / len(locationy)
            avg_roundy = round(avgy,1)
            print("average value of y", avg_roundy*0.01)
            a1 = 0.185
            a2 = 0.155
            x = avg_roundz*0.01
            y = avg_roundy*0.01
            ...
            print("servo 0 angle", q1)
            print("servo 1 angle", q2)
            if (q1 and q2) != "nan": 
                print("something")

                angle_value_list = [str(servo_0_angle),str(servo_1_angle)]    
                send_string = ','.join(angle_value_list)
                send_string += "\n"
                #ser.write(str.encode(str(q1)))
